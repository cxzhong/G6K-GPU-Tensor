...
class CustomBuildExt(_build_ext):
    ...
    def run(self):
        print("Starting CustomBuildExt.run()...")
        
        # Install system dependencies first
        try:
            self.install_system_deps()
        except Exception as e:
            print(f"Warning: System dependencies installation failed: {e}")
        ...
        print("Dependencies setup complete, starting extension build...")
...
def get_ext_modules():
    ...
    # Check that required Cython source files exist
    required_files = [
        'g6k/siever.pyx',
    ...
    if missing_files:
        print(f"Error: Missing required Cython source files: {missing_files}")
        return []
    else:
        print(f"✅ Found required Cython source files: {required_files}")
    ...
    try:
        ...
        # Build siever_params with same configuration as siever
        print("Configuring siever_params extension...")
        ext_modules.append(Extension(
            'g6k.siever_params',
        ...
        print(f"Successfully configured {len(ext_modules)} extensions:")
        for ext in ext_modules:
            print(f"  - {ext.name}")
        
    except Exception as e:
        print(f"Error: Could not configure C++ extensions: {e}")
        import traceback
        traceback.print_exc()
        print("This will likely cause import errors!")
        
        # Don't just silently continue - we need these extensions
        # Try to build minimal standalone extensions
        try:
            print("Attempting minimal standalone extension build...")
            ...
            # Try to add cysignals if available
            try:
                import cysignals
                basic_includes.append(os.path.dirname(cysignals.__file__))
            except ImportError:
                print("Warning: cysignals not available")
            ...
            print(f"Minimal standalone extensions configured: {[ext.name for ext in ext_modules]}")
            
        except Exception as fallback_e:
            print(f"Even minimal fallback failed: {fallback_e}")
            import traceback
            traceback.print_exc()
            print("\n" + "="*50)
            print("CRITICAL ERROR: Cannot build any extensions!")
            print("This will cause ModuleNotFoundError at runtime!")
            print("="*50 + "\n")
            # Return empty list - setup will continue but runtime will fail
            ext_modules = []

    # Apply cythonize to all extensions
    if ext_modules:
        print(f"Applying cythonize to {len(ext_modules)} extensions...")
        try:
            ...
            print("Cythonize completed successfully")
        except Exception as e:
        ...
        print("Warning: No extensions to build!")

    return ext_modules

if __name__ == "__main__":
    print("Starting G6K GPU Tensor setup...")
    ext_mods = get_ext_modules()
    print(f"Final extension modules: {[ext.name if hasattr(ext, 'name') else str(ext) for ext in ext_mods]}")
