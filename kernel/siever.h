...
// assumed to be at most OTF_LIFT_HELPER_DIM
#ifndef DUAL_HELPER_DIM
#define DUAL_HELPER_DIM 16
#endif

#ifndef DUAL_HELPER_VECS
#define DUAL_HELPER_VECS 64
#endif
*/
...
static constexpr unsigned int XPC_WORD_LEN = (POPCOUNT)? 4 : 0; // number of 64-bit words of simhashes
static constexpr unsigned int XPC_BIT_LEN = XPC_WORD_LEN*64; // number of bits for simhashes
...
static constexpr unsigned int YR_DIM = (NOYR)?0:MAX_SIEVING_DIM;
static constexpr unsigned int DUAL_HELPER_DIM = (NOYR)?0:20;
...
template<size_t tuple_size> struct Qtuple;
template<size_t tuple_size> struct Ltuple;
struct queues;
class Siever;
class UidHashTable;
class SimHashes;
...
public:
  ...
  inline bool check_uid_unsafe(UidType uid);
  ...
  inline const std::vector<UidType>& get_uid_coeffs() const; 

  std::vector<UidType> uid_coeffs;        // description of the hash function.
private:
    // internal reserve function (to be called inside public reserve() and reset_hash_function())
    ...
    struct padded_map: phmap::flat_hash_set<UidType> { cacheline_padding_t pad; };
    ...
    std::array<padded_mutex, DB_UID_SPLIT> db_mut; // array of mutexes. db_mut[i] protects all access to db_uid[i].
...
public:
    ...
    inline CompressedVector compress(std::array<LFT,YR_DIM> const &yr) const;
};
...
class DualHashes
{
    private:
        ...
        std::vector<std::vector<LFT>> dual_vecs;
    ...
    public:
        ...
        inline double get_acceptance_ratio();
        inline DualHashVector compress(std::array<LFT,OTF_LIFT_HELPER_DIM> const &yr) const;
        inline int sqdist(const DualHashVector &a, const DualHashVector &b, const bool sign) const;
        inline int sqlen(const DualHashVector &a) const;
        inline bool test(const DualHashVector &a, const DualHashVector &b, const bool sign);
        inline LFT get_dual_vecs( int i, int j ) const;
        inline unsigned int get_nr_vecs() const;
        inline unsigned int get_dim_vecs() const;
        inline unsigned int get_dh_bound() const; 
};
...
struct Entry
{
    ...
    std::array<LFT,YR_DIM> yr;     // Vector coordinates in gso basis renormalized by the rr[i] (for faster inner product)
...
struct CompressedEntry {
    ...
    std::uint8_t bucket_center_count; // number of times this has been used as bucket center
};

// Define an ordering for sorting Compressed Entries, used for sorting.
struct compare_CE
{
    bool operator()(CompressedEntry const& lhs, CompressedEntry const& rhs) const { return lhs.len < rhs.len; }
};
struct compare_E
{
    bool operator()(Entry const& lhs, Entry const& rhs) const { return lhs.len < rhs.len; }
};
...
public:
  ...
  std::string simhash_codes_basedir = ".";  // directory holding spherical codes for simhash.
    ...
    size_t dh_dim = 16;
    size_t dh_vecs = 64;
    float dh_bucket_ratio = 0.5;
    size_t multi_bucket = 4;
    size_t max_nr_buckets = 64*1024;
    float lenbound_ratio = 0.85;
   
    size_t bdgl_bucket_size = 16*1024;
    std::string gpu_bucketer = "bgj1";
    bool gpu_triple = true;
...
public:
    ...
    */
    ...
    void reset_dual_vecs( LFT* dual_vecs_ptr, size_t nr_dual_vecs, size_t dim_dual, LFT conv_ratio, unsigned int target_index, float max_hbound ) 
    {
        if( nr_dual_vecs == 0 ) {
            ...
            if( !NOYR ) {
                ...
                {
                    e.dual_helper = dual_hashes.compress( e.otf_helper );
                } );
            }
    ...
    inline void statistics_lift_hash( unsigned int N, float* liftlen, float* hashlen ); 
    ...
    void output_bench() {
        std::cerr << "EL bench: " << std::endl 
                  << "vecs: " << benchEL_vecs << std::endl 
                  << "EL_init: " << benchEL_init << std::endl 
                  << "EL_recomp: " << benchEL_recomp << std::endl 
                  << "EL_refresh: " << benchEL_refresh << std::endl 
                  << "EL_grow: " << benchEL_grow << std::endl 
                  << "IN_post: " <<  benchEL_postinsert << std::endl
                  << "B_prepare: " << benchB_prepare << std::endl
                  << "B_bucketing: " << benchB_bucketing << std::endl
                  << "B_gather: " << benchB_gather << std::endl
                  << "B_merge: " << benchB_merge << std::endl;
    }
...
*/
    ...
    FT get_lift_bound(unsigned int i) const;
...
// GPU sieve
    ...
    void gpu_general_smartInit();
    void gpu_general_freeAll();

    void global_init_gpus();
    void dh_init_gpus();

    void gpu_bucketing_prepare( const size_t threads, const std::vector<size_t>& b_idxs );
    ...
    void gpu_extend_task( const size_t A, const size_t extend_left, const size_t t_id, const size_t threads, const size_t chunk_size);
    void gpu_extend( const size_t A, const size_t extend_left, const size_t threads );

    size_t gpu_stats_nonduplicates;
    size_t gpu_stats_inserted;

    //extend left bench
    double benchEL_vecs = 0.;
    double benchEL_init = 0.;
    double benchEL_recomp = 0.;
    double benchEL_refresh = 0.;
    double benchEL_grow = 0.;
    double benchEL_postinsert = 0.;
    double benchB_prepare = 0.;
    double benchB_bucketing = 0.;
    double benchB_gather = 0.;
    double benchB_merge = 0.;
public:
