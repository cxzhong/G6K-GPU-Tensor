...
{
    // always true if popcount disabled
    if( XPC_WORD_LEN==0 )
        return true;
...
{
    ATOMIC_CPUCOUNT(214);
    bool constexpr rec_yr = (!NOYR) and ((what_to_recompute & Recompute::recompute_yr) != Recompute::none);
    bool constexpr rec_len = (what_to_recompute & Recompute::recompute_len) != Recompute::none;
    bool constexpr rec_c = (XPC_WORD_LEN>0) and ((what_to_recompute & Recompute::recompute_c) != Recompute::none);
    ...
    bool constexpr rec_otf_helper = (!NOYR) and ((what_to_recompute & Recompute::recompute_otf_helper) != Recompute::none);
    ...
    {
        for (int k = 0; k < int(OTF_LIFT_HELPER_DIM); ++k)
        {
            int const i = l - (k + 1);
...
inline void Siever::recompute_data_for_entry_babai(Entry &e, int babai_index)
{
  ...
  // recompute y, yr, len for the other indices (if requested)
    ...
    for (int i = babai_index -1 ; i >= 0; --i)
    {
        ...
        yi += c;
        yi *= sqrt_rr[i];
        CPP17CONSTEXPRIF (rec_yr) e.yr[i] = yi; // ( original_value(yi) + c ) * sqrt_rr[i]. Note that assignment loses precision.
